...
def main() -> int:
    parser = argparse.ArgumentParser(description="Preview cleaned CV text or send it to DeepSeek for human-reviewed scoring.")
    parser.add_argument("file", type=Path)
    parser.add_argument("--name", required=True)
    parser.add_argument("--address", default="")
    parser.add_argument("--graduation-year", type=int, action="append", default=[])
    parser.add_argument("--send", action="store_true", help="Send cleaned CV to DeepSeek (may incur API usage).")
    ...
    try:
        if not args.send:
            ...
            print("\nReview this text. Add --send to send it to DeepSeek.", file=sys.stderr)
        else:
