...
export function CriterionCard({ c }) {
  ...
  return (
    <article className={`criterion-card ${isMet ? 'met' : isPartial ? 'partial' : 'notfound'}`}>
      ...
      <div className={`quote-block ${c.evidence_quote ? 'has-quote' : ''}`}>
    ...
    <div>
      <div className="flex items-end justify-between pb-5 border-b border-slate-200 mb-5">
        <div>
          ...
          <div className="font-mono text-[32px] font-bold tracking-tight text-slate-900">
      ...
      <div className="mt-5 p-3 bg-slate-50 border border-slate-200 rounded-md">
        <strong className="text-xs text-slate-500 uppercase block mb-1">Scoring summary and notes</strong>
